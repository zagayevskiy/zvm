...
internal class ZcParserTest(private val test: TestData) {
    ...
    companion object {
        ...
        fun data() = listOf(
                ...
                "fn f() { sizeof<int>; }" expect program(fn("f", args = emptyList(), body = AstBlock(listOf(AstExpressionStatement(AstSizeOf(UnresolvedType.Simple("int"))))))),
                "fn f(i: int): byte { return cast<byte>(i); }" expect program(fn("f", args = listOf(Arg("i") withType "int"), returnType = "byte".type, body = AstBlock(listOf(AstFunctionReturn(AstHardCastExpr("i".id, UnresolvedType.Simple("byte"))))))),
