...
class GraphVizGenerator(private val ast: Ast) {
    ...
    private val visitor = object : AstVisitor<Pair<String, Int>> {
        ...
        override fun visit(ast: AstWhen) = "when" to ++id

        override fun visit(ast: AstWhenBranch) = "when-branch" to ++id
        ...
        override fun visit(ast: AstDefinedStruct) = "struct ${ast.name}: ${ast.structType}" to ++id

        override fun visit(ast: AstStructFieldDereference) = ".${ast.name}" to ++id

        override fun visit(ast: AstUnknownFunctionReference) = "::${ast.name}" to ++id
